...
local start = date().usec,
    ...
    function T(expected, value){
        ...
        b.DP = 20
        b.RM = 1
        assert(expected.tostring(), Big(value).abs().toString());
    }

    server.log("\n Testing abs...");

    T(0.00001, "-1e-5");    //TODO: Bad test?
    T(9000000000, "-9e9");  //Bigger than our float can support
    ...
    assert(true, isMinusZero(Big("-0")));   //TODO: BUG: this is failing
    // assert(true, isMinusZero(Big(minusZero)));
    assert(false, isMinusZero(Big(-0).abs()));
    // assert(false, isMinusZero(Big(minusZero).abs()));
    assert(true, !isMinusZero(Big("-0").abs()));
    // assert(true, !isMinusZero(Big(minusZero).abs()));
